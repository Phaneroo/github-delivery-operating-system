...
- **Labels set up?** `gh label list --repo <owner>/<repo>` — look for `production`, `qa`, `qa-request`, `sprint`, `sprint-child`, `planning`, `declined`, `ready-for-deploy`, `delivery-ops-filed`, `qa-rollup`. Missing labels mean `Setup Labels` hasn't been run there yet — offer to fix it directly rather than just reporting the gap: `gh workflow run setup-labels.yml --repo <owner>/<repo>` (it's a `workflow_dispatch` trigger, so this actually creates them on the spot). Confirm with the user first since it's a real change to their repo. (`sprint-active` instead of `sprint-child` means the repo is on a pre-1.5.0 install — still valid, just the older label name.)
- **Repo variables set?** `gh variable list --repo <owner>/<repo>` — look for `RELEASE_APPROVER`, `QA_APPROVER`, `QA_ASSIGNEES`. If unset, say so plainly: the issue will still get created, but `notify-release-approver` will ping the literal placeholder `release-approver`/`qa-approver`, not a real person. Setting these requires repo admin access (`gh variable set NAME --repo <owner>/<repo> --body <value>`) — don't set them without being asked to, since they name a real person as approver. Also note `DELIVERY_OS_AUTO_QA_MODE` (`rolling` default / `per-change` / `off`), the 1.8.0 `DELIVERY_OS_AUTO_QA` (`all` → per-change, `pr-only` → direct pushes add nothing, `off`), `DELIVERY_OS_AUTO_TASK` (per-change only: `false` = direct pushes file no synthetic Task), `DELIVERY_OS_AUTO_QA_QUIET_PATHS` (globs that file nothing; defaults to docs/settings) and `DELIVERY_OS_AUTO_CLOSE` (`false` = releases don't close auto-filed issues) if set — they explain why `auto-qa-request` did or didn't file something. If a repo that pushes straight to `main` is drowning in auto-filed Task + QA Request pairs, it's on per-change mode (explicitly, or via `DELIVERY_OS_AUTO_QA=all`): suggest the rolling default (unset those, or set `DELIVERY_OS_AUTO_QA_MODE=rolling`), `pr-only` or `DELIVERY_OS_AUTO_TASK=false`, linking pushes via `Refs #N` in the commit message, or `[skip qa-request]` for trivial commits — but don't set the variables without being asked.
- **Up to date?** Compare the installed version against the latest published one: `gh api repos/<owner>/<repo>/contents/.github/delivery-os.json --jq '.content' | base64 -d | jq -r .version` vs. `npm view github-delivery-os version`. If installed is behind, say so before doing anything else and offer to update (`npx github-delivery-os@latest install --with-templates --with-skill --with-labels --update .`, run from a local clone) — confirm first, same as the label-setup fix above, since it rewrites installed workflow/template files. If the session opened with a "Delivery OS X installed, Y available" message, the repo's update-check hook already did this comparison — make that same offer once, rather than re-checking or asking twice. A missing `delivery-os.json` (pre-manifest install, or one done via `scripts/install.sh`, which doesn't write one) isn't itself a problem — skip this check silently rather than treating "no manifest" as "not installed"; the "Installed?" check above already covers that.
...
- **Approve or decline:** `qa-rollup-approval` acts only on the `QA_APPROVER` login, with the same login check as "Commenting as an approver" above. The comment must *start* with a phrase or emoji:
  - Approve: `qa approved`, `approved`, `qa ok`, `looks good`, `lgtm`, `all good`, `good to go`, `ship it` (anything may follow), or just `ok`, `approve`, `tested` or `passed` as the whole comment ("Tested!" counts; "Ok, I'll test tomorrow" doesn't), or ✅ 👍 ✔️. The approver ticking **Approved: all changes above have been tested** also approves. Approval ticks every line and closes the issue.
  - Decline: `not approved`, `declined`, `decline`, `rejected`, `reject`, `failed`, `changes needed`, `needs work`, `not ok`, `blocked`, or ❌ 👎 🚫. Declining keeps the issue open with QA Outcome *Fail*, and later fixes append to it.
  - Decline phrases are checked first. The latest verdict wins: a decline after approval reopens the issue. Emoji **reactions** don't count; the emoji must be in a comment. Anyone else's verdict comment gets a "only the QA approver can…" reply and is ignored, and a non-approver's box tick is reverted.
  - These are broader than the release gate's keywords above; don't mix the two up. `lgtm` approves the rolling issue but not a release.
- **Only approve or decline when the user asks you to, as the approver.** Approving tells the team the whole list was tested. Never do it on your own initiative, even if the changes look fine.
- **Plain English:** the indented bullets under a line are that change's draft summary. Offer to rewrite them from the diff per "Writing the What Changed changelog" below. Show the proposed text first, and edit only those bullets with `gh issue edit --body-file`, keeping the head line and its marker. There's no per-change review box in rolling mode; the approver's approval covers the list.
- **Status:** when asked what's awaiting QA, read the open rolling issue: how many lines, which are ticked, whether it was declined (QA Outcome *Fail*), and how long it has been open.
...
- **Open QA requests:** `gh issue list --repo <owner>/<repo> --label qa-request --state open` (includes the rolling QA issue, if any — see "The rolling QA issue")
- **The rolling QA issue:** `gh issue list --repo <owner>/<repo> --label qa-rollup --state open`
