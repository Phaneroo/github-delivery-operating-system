...
- Direct-push QA Requests now list the pushed files. Push file lists come from the compare API, because the push payload GitHub Actions delivers carries no `added`/`modified`/`removed` lists. This was found in an end-to-end run on a real repo; without it, the docs-only skip never fired for direct pushes.
