...
6. **Validate before committing:**
   ...
   - If a workflow was added/removed/renamed, confirm `package.json`'s `files` array still lists exactly the intended `.github/workflows/*.yml` paths — it lists them explicitly rather than the whole directory (so this repo's own `ci.yml`/`release.yml`/`pages.yml` don't get bundled into the npm tarball), which means it doesn't update itself when `WORKFLOWS` in `src/install.js` changes. Worth a `npm pack --dry-run` to actually look at what would ship, not just trust the config — that's exactly how the `ci.yml`/`release.yml`/`pages.yml` bloat was caught in the 1.2.0 release, after it had already shipped once in 1.1.0.
   - If `.claude/skills/delivery-ops/SKILL.md` changed: `npm run check-skill-sync` — this repo's bundled copy and the canonical one in the `jkaweesi22/KaweesiSkills` repo are two separate files that can silently drift. That script fetches the canonical copy via `gh api` (works because `gh`'s current auth already has read access to that private repo) and diffs it against the local copy. It's local-only, not a CI check — `KaweesiSkills` is private, so CI runners have no credentials to fetch it. If they've diverged, decide which side is right and copy it over the other before proceeding.
...
- `npm run check-skill-sync` (`scripts/check-skill-sync.sh`) checks `.claude/skills/delivery-ops/SKILL.md` against its canonical copy in the private `jkaweesi22/KaweesiSkills` repo. Not run in CI (no credentials there for a private repo) — it's a manual step, worth running whenever that file changes, not just at release time.
- After a real `npm publish`, worth actually testing the published package once — `npx github-delivery-os@<version> install --with-skill .` in a scratch directory — rather than trusting that local testing generalizes. This caught nothing wrong in the 1.2.0 release, but it's what actually proved the `ci.yml`/`release.yml`/`pages.yml` packaging fix took effect and that `--with-skill` works from the real registry tarball, not just local `node bin/delivery-os.js` runs.
