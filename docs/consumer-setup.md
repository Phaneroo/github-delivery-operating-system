...
| `DELIVERY_OS_AUTO_QA` | Optional; the 1.8.0 setting, still honored when `DELIVERY_OS_AUTO_QA_MODE` is unset: `all` → per-change, `pr-only` → direct pushes add nothing (PRs still do), `off` → nothing is filed |
...
- **Release roll-up.** A Production Release's *What's in this release* comment lists every rolling-issue line (the open issue, plus ones approved since the last release) with its QA status.
- **Switching back.** Set `DELIVERY_OS_AUTO_QA_MODE=per-change` (or keep `DELIVERY_OS_AUTO_QA=all`) for one QA Request per change. Set it to `off` to file nothing.
