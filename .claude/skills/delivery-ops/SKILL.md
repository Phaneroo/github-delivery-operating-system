...
- **Approve or decline:** `qa-rollup-approval` acts only on the `QA_APPROVER` login, with the same login check as "Commenting as an approver" above. The comment must *start* with a phrase or emoji:
  - Approve: `qa approved`, `approved`, `approve`, `qa ok`, `ok`, `looks good`, `lgtm`, `all good`, `tested`, `passed`, `good to go`, `ship it`, or ✅ 👍 ✔️. The approver ticking **Approved: all changes above have been tested** also approves. Approval ticks every line and closes the issue.
